...
#   - peer - builds a native blockchain peer binary
#   - orderer - builds a native blockchain orderer binary
...
CHAINTOOL_URL ?= https://nexus.mcc-github.org/content/repositories/releases/org/mcc-github/blockchain/mcc-github-blockchain/chaintool-$(CHAINTOOL_RELEASE)/mcc-github-blockchain-chaintool-$(CHAINTOOL_RELEASE).jar
...
docker-thirdparty:
	docker pull $(BASE_DOCKER_NS)/blockchain-couchdb:$(BASE_DOCKER_TAG)
	docker tag $(BASE_DOCKER_NS)/blockchain-couchdb:$(BASE_DOCKER_TAG) $(DOCKER_NS)/blockchain-couchdb
	docker pull $(BASE_DOCKER_NS)/blockchain-zookeeper:$(BASE_DOCKER_TAG)
	docker tag $(BASE_DOCKER_NS)/blockchain-zookeeper:$(BASE_DOCKER_TAG) $(DOCKER_NS)/blockchain-zookeeper
	docker pull $(BASE_DOCKER_NS)/blockchain-kafka:$(BASE_DOCKER_TAG)
	docker tag $(BASE_DOCKER_NS)/blockchain-kafka:$(BASE_DOCKER_TAG) $(DOCKER_NS)/blockchain-kafka
...
linter: check-deps buildenv
	...
	@$(DRUN) $(DOCKER_NS)/blockchain-buildenv:$(DOCKER_TAG) ./scripts/golinter.sh

check-deps: buildenv
	@echo "DEP: Checking for dependency issues.."
	@$(DRUN) $(DOCKER_NS)/blockchain-buildenv:$(DOCKER_TAG) ./scripts/check_deps.sh

check-metrics-doc: buildenv
	@echo "METRICS: Checking for outdated reference documentation.."
	@$(DRUN) $(DOCKER_NS)/blockchain-buildenv:$(DOCKER_TAG) ./scripts/metrics_doc.sh check

generate-metrics-doc: buildenv
	@echo "Generating metrics reference documentation..."
	@$(DRUN) $(DOCKER_NS)/blockchain-buildenv:$(DOCKER_TAG) ./scripts/metrics_doc.sh generate
...
$(BUILD_DIR)/images/baseos/$(DUMMY):
	...
	$(DBUILD) -f images/peer/Dockerfile \
		...
		-t $(BASE_DOCKER_NS)/blockchain-$(TARGET) images/peer
	docker tag $(BASE_DOCKER_NS)/blockchain-$(TARGET) \
		$(DOCKER_NS)/blockchain-$(TARGET):$(BASE_VERSION)
	docker tag $(BASE_DOCKER_NS)/blockchain-$(TARGET) \
		$(DOCKER_NS)/blockchain-$(TARGET):$(DOCKER_TAG)
	@touch $@
...
$(BUILD_DIR)/images/%/$(DUMMY):
	...
	$(DBUILD) -f images/$(TARGET)/Dockerfile \
		--build-arg GO_VER=${GO_VER} --build-arg ALPINE_VER=${ALPINE_VER} \
		${BUILD_ARGS} \
		-t $(BASE_DOCKER_NS)/blockchain-$(TARGET) .
	docker tag $(BASE_DOCKER_NS)/blockchain-$(TARGET) \
		$(DOCKER_NS)/blockchain-$(TARGET):$(BASE_VERSION)
	docker tag $(BASE_DOCKER_NS)/blockchain-$(TARGET) \
		$(DOCKER_NS)/blockchain-$(TARGET):$(DOCKER_TAG)
...
dist/%: release/%
	...
	cd release/$(@F) && tar -czvf mcc-github-blockchain-$(@F).$(PROJECT_VERSION).tar.gz *

.PHONY: protos
protos: buildenv
	@$(DRUN) $(DOCKER_NS)/blockchain-buildenv:$(DOCKER_TAG) ./scripts/compile_protos.sh

%-docker-list:
	$(eval TARGET = ${patsubst %-docker-list,%,${@}})
	@echo $(DOCKER_NS)/blockchain-$(TARGET):$(DOCKER_TAG)

docker-list: $(patsubst %,%-docker-list, $(IMAGES))

%-docker-clean:
	$(eval TARGET = ${patsubst %-docker-clean,%,${@}})
	-docker images --quiet --filter=reference='$(DOCKER_NS)/blockchain-$(TARGET):$(ARCH)-$(BASE_VERSION)$(if $(EXTRA_VERSION),-snapshot-*,)' | xargs docker rmi -f
...
%-docker-tag-latest:
	...
	docker tag $(DOCKER_NS)/blockchain-$(TARGET):$(DOCKER_TAG) $(DOCKER_NS)/blockchain-$(TARGET):latest

docker-tag-stable: $(IMAGES:%=%-docker-tag-stable)

%-docker-tag-stable:
	$(eval TARGET = ${patsubst %-docker-tag-stable,%,${@}})
	docker tag $(DOCKER_NS)/blockchain-$(TARGET):$(DOCKER_TAG) $(DOCKER_NS)/blockchain-$(TARGET):stable
...
clean-all: clean gotools-clean dist-clean
	-@rm -rf /var/mcc-github/*
	-@rm -rf docs/build/

.PHONY: dist-clean
dist-clean:
	-@rm -rf release/windows-amd64/mcc-github-blockchain-windows-amd64.$(PROJECT_VERSION).tar.gz
	-@rm -rf release/darwin-amd64/mcc-github-blockchain-darwin-amd64.$(PROJECT_VERSION).tar.gz
	-@rm -rf release/linux-amd64/mcc-github-blockchain-linux-amd64.$(PROJECT_VERSION).tar.gz
	-@rm -rf release/linux-s390x/mcc-github-blockchain-linux-s390x.$(PROJECT_VERSION).tar.gz
	-@rm -rf release/linux-ppc64le/mcc-github-blockchain-linux-ppc64le.$(PROJECT_VERSION).tar.gz
