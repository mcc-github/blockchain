...
func TestCompositeKeyConstruction(t *testing.T) {
	...
	for _, testDatum := range testData {
		...
		assert.Equal(t, bytes.Compare(rangeScan.startKey, key), -1) 
		assert.Equal(t, bytes.Compare(rangeScan.endKey, key), 1)    
	}
	...
	for i, testDatum := range testData {
		for j, another := range testData {
			...
			assert.False(t, bytes.Compare(anotherKey, rangeScan.startKey) == 1 && bytes.Compare(anotherKey, rangeScan.endKey) == -1) 
		}
	}
}
