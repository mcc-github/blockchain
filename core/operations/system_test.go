...
import (
	...
	"github.com/mcc-github/blockchain/core/operations/fakes"
...
var _ = Describe("System", func() {
	...
	Context("when the metrics provider is prometheus", func() {
		...
		It("records the blockchain version", func() {
			err := system.Start()
			Expect(err).NotTo(HaveOccurred())
			...
			resp.Body.Close()
			Expect(err).NotTo(HaveOccurred())
			Expect(string(body)).To(ContainSubstring("# TYPE blockchain_version gauge"))
			Expect(string(body)).To(ContainSubstring(`blockchain_version{version="test-version"}`))
		})
	})
	...
	Context("when the metrics provider is statsd", func() {
		...
		It("emits the blockchain version statsd metric", func() {
			statsBuffer := gbytes.NewBuffer()
			go recordStats(statsBuffer)

			err := system.Start()
			Expect(err).NotTo(HaveOccurred())
			Eventually(statsBuffer).Should(gbytes.Say(`\Qprefix.blockchain_version.test-version:1.000000|g\E`))
		})
