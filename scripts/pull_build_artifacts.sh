...
# Push tagged images to mcc-github dockerhub
#############################################

export ORG_NAME=mcc-github/blockchain
export NEXUS_URL=nexus3.mcc-github.org:10001
...
printHelp() {
  ...
  echo
  echo "pull_All - pull blockchain, blockchain-ca and thirdparty images"
  echo "pull_Fabric_Images_All_Platforms - pull blockchain images amd64, s390x"
  echo "cleanup - delete unused docker images"
  echo "pull_Fabric_Images - pull blockchain docker images on current arch"
  echo "pull_Fabric_Binaries - pull blockchain binaries on current arch"
  echo "pull_Thirdparty_Images - pull blockchain thirdparty docker images on current arch"
  echo "pull_Ca_Images - pull blockchain ca docker images on current arch"
  echo
...
# pull blockchain, blockchain-ca and thirdparty images, and binaries
pull_All() {
    ...
    echo "-------> pull blockchain docker images"
    pull_Fabric_Images
    echo "-------> pull blockchain ca docker images"
    pull_Ca_Images
}

# pull blockchain docker images
pull_Fabric_Images() {
...
pull_Fabric_Binaries() {
    ...
    MVN_METADATA=$(echo "https://nexus.mcc-github.org/content/repositories/releases/org/mcc-github/blockchain/mcc-github-blockchain-$STABLE_VERSION/maven-metadata.xml")
    ...
    echo "-------> COMMIT:" $COMMIT
    curl https://nexus.mcc-github.org/content/repositories/releases/org/mcc-github/blockchain/mcc-github-blockchain-$STABLE_VERSION/$MARCH.$STABLE_VERSION-$COMMIT/mcc-github-blockchain-$STABLE_VERSION-$MARCH.$STABLE_VERSION-$COMMIT.tar.gz | tar xz
    if [ $? != 0 ]; then
	echo "-------> FAILED to pull blockchain binaries"
        exit 1
    fi
}

# pull blockchain docker images from amd64 and s390x platforms
pull_Fabric_Images_All_Platforms() {

    # pull stable images from nexus and tag to mcc-github
...
pull_Thirdparty_Images() {
    ...
    BASEIMAGE_VERSION=$(curl --silent  https://raw.githubusercontent.com/mcc-github/blockchain/master/Makefile 2>&1 | tee Makefile | grep "BASEIMAGE_RELEASE=" | cut -d "=" -f2)
...
# pull blockchain-ca docker images
pull_Ca_Images() {
