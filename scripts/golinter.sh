...
OUTPUT="$(go list -f "$TEMPLATE" ./... | grep -Ev '^github.com/mcc-github/blockchain/protos($|/)' | grep 'golang.org/x/net/context' | cut -f1 -d:)"
if [ -n "$OUTPUT" ]; then
...
#   FAB-12082 - gossip
#   FAB-12083 - orderer
OUTPUT="$(go vet -lostcancel=true ./... 2>&1 | grep -Ev '^#|(blockchain/)?gossip/comm/comm_impl.go|(blockchain)?orderer/common/cluster/comm.go' || true)"
if [ -n "$OUTPUT" ]; then
    echo "The following files contain go vet errors"
    echo $OUTPUT
    exit 1
