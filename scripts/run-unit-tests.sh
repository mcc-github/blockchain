...
serial_packages=(
    "github.com/mcc-github/blockchain/gossip/..."
)
...
packages_diff() {
    git -C "${GOPATH}/src/github.com/mcc-github/blockchain" diff --no-commit-id --name-only -r "${1:-HEAD}" |
        (grep '.go$' || true) | \
        sed 's%/[^/]*$%%' | sort -u | \
        awk '{print "github.com/mcc-github/blockchain/"$1}'
}
...
main() {
    ...
    if [ "$(uname -m)" == "ppc64le" ] || [ "$(uname -m)" == "s390x" ]; then
        excluded_packages+=("github.com/mcc-github/blockchain/core/chaincode/platforms/java")
    fi

    # default behavior is to run all tests
    local -a package_spec=("${TEST_PKGS:-github.com/mcc-github/blockchain/...}")
