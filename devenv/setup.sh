...
DEVENV_REVISION=`(cd /mcc-github/blockchain/devenv; git rev-parse --short HEAD)`
...
# ----------------------------------------------------------------
runuser -l ubuntu -c '/mcc-github/blockchain/devenv/install_nvm.sh'
...
sudo mkdir -p /var/mcc-github
sudo chown -R ubuntu:ubuntu /var/mcc-github
...
sudo cp /mcc-github/blockchain/devenv/limits.conf /etc/security/limits.conf

# Configure vagrant specific environment
cat <<EOF >/etc/profile.d/vagrant-devenv.sh
# Expose the devenv/tools in the $PATH
export PATH=\$PATH:/mcc-github/blockchain/devenv/tools:/mcc-github/blockchain/.build/bin
export FABRIC_CFG_PATH=/mcc-github/blockchain/sampleconfig/
export VAGRANT=1
export CGO_CFLAGS=" "
EOF

# Set our shell prompt to something less ugly than the default from packer
# Also make it so that it cd's the user to the blockchain dir upon logging in
cat <<EOF >> /home/ubuntu/.bashrc
PS1="\u@mcc-github-devenv:$DEVENV_REVISION:\w$ "
cd $GOPATH/src/github.com/mcc-github/blockchain/
EOF
