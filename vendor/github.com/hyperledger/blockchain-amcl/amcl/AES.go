...
var aes_InCo = [...]byte {0xB,0xD,0x9,0xE}  
...
func aes_product(x uint32,y uint32) byte { 
        xb:=aes_unpack(x)
        yb:=aes_unpack(y)
...
func aes_InvMixCol(x uint32) uint32 { 
        var b [4]byte
        m:=aes_pack(aes_InCo)
        b[3]=aes_product(m,x)
        m=aes_ROTL24(m)
